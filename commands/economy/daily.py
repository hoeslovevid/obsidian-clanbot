...
async def _run_daily(interaction: discord.Interaction, force_reset: bool = False):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row:
            ...
            if last_claim_date == today:
                ...
                if freeze_used_month != current_month:
                    embed.add_field(
                        name="🛡️ Streak Freeze",
                        value="You have **1 monthly freeze** if you miss tomorrow — streak preserved once per month.",
                        inline=False,
                    )
                elif freeze_used_month == current_month:
                    embed.add_field(
                        name="🛡️ Streak Freeze",
                        value="Already used this month — resets on the 1st.",
                        inline=False,
                    )
    ...
    try:
        ...
        hint = await get_next_step_hint(interaction.guild.id, interaction.user.id, "daily")
        if hint:
            desc += f"\n\n-# {hint}"
    except Exception:
        pass
